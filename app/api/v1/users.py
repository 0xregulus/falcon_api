...
from app.utils.aes import get_aes_key, generate_aes_iv
    ...
    def process_login(self, req, res):
        ...
        try:
            ...
            if verify_password(password, user_db.password.encode('utf-8')):
                session_data = user_db.to_dict()
                session_data['key'] = get_aes_key(password)
                session_data['iv'] = generate_aes_iv()
                self.on_success(res, session_data)
            else:
